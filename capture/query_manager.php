...
function getBins() {
    ...
    foreach ($bin_results as $data) {
        ...
        if ($bin->type == "track" || $bin->type == "search") {
...
function getSearchBins(){
    $dbh = pdo_connect();
    $sql = 'SELECT `tcat_query_bins`.`id`, `tcat_query_bins`.`querybin`, `description`, `origin_phrase`, `createtime`, `updatetime`
            FROM `tcat_query_bins`
            INNER JOIN `tcat_search_timetable` ON `tcat_search_timetable`.`querybin_id` = `tcat_query_bins`.`id`';
    $rec = $dbh->prepare($sql);
    $rec->execute();
    ...
    foreach ($rs_sbin as $data) {
        if (!isset($querybins[$data['id']])) {
            ...
            $bin->description = $data['description'];
...
function getSavedBins(){
    ...
    $sql = 'SELECT `tcat_query_bins`.`id`, `tcat_query_bins`.`querybin`, `description`, `origin_phrase`, `createtime`, `savedtime`
            FROM `tcat_query_bins`
            INNER JOIN `tcat_search_saved_archive` ON `tcat_search_saved_archive`.`querybin_id` = `tcat_query_bins`.`id`';
    $rec = $dbh->prepare($sql);
    $rec->execute();
    $rs_sbin = $rec->fetchAll();
    $querybins = array();
